...
class Payment_Methods(db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint(
            'expiration_month >= 1 AND expiration_month <= 12', name='check_expiration_month'),
...
class Event(db.Model):
    ...
    event_type = db.Column(ENUM(
        'Concert', 'Game', 'Conference', 'Convention', 'Show', 'Other'), nullable=False)
...
class Ticket(db.Model):
    ...
    event_id = db.Column(db.Integer, db.ForeignKey(
        'Event.event_id', ondelete='CASCADE'), nullable=False)
    seat_number = db.Column(db.String(5))
    price = db.Column(db.Integer, nullable=False)
    status = db.Column(ENUM('Sold', 'Pending', 'Avalible'), nullable=False)
...
class Transaction(db.Model):
    ...
    payment_method_id = db.Column(db.Integer, db.ForeignKey(
        'Payment_Methods.payment_method_id'), nullable=False)
