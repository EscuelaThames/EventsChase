...
def create_user():
    ...
    new_user = Users(first_name=first_name, last_name=last_name, email=email, address=address, phone_number=phone_number)
...
def create_credentials(user_id):
    ...
    credentials = Credentials(user_id=user_id, username=username, password_hash=password_hash)
...
def read_credentials():
    ...
    if submitted_credentials:
        return jsonify({"message": "Credentials accepted", "userID": submitted_credentials.user_id}), 200
    else:
...
def create_payment_method(user_id):
    ...
    payment_method = Payment_Methods(user_id=user_id, card_number=card_number, expiration_month=expiration_month, expiration_year=expiration_year, cvv=cvv, billing_address=billing_address)
...
def update_payment_method(payment_method_id):
    ...
    if not payment_method:
     ...
     # examining the json request and updating any values that could change
    data = request.json
    payment_method.card_number = data.get("cardNumber", payment_method.card_number)
    payment_method.expiration_month = data.get("expirationMonth", payment_method.expiration_month)
    payment_method.expiration_year = data.get("expirationYear", payment_method.expiration_year)
    payment_method.cvv = data.get("cvv", payment_method.cvv)
    payment_method.billing_address = data.get("billingAddress", payment_method.billing_address)
...
def create_event():
    ...
    new_event = Event(event_name=event_name, event_date=event_date, event_type=event_type, capacity=capacity)
    ...
    try:
        ...
        for i in range(1, capacity + 1):
            ticket = Ticket(event_id=new_event.event_id, seat_number=str(i))
...
def list_events():
    ...
    events_list = [{"eventName": event.event_name, "eventDate": event.event_date, "eventType": event.event_type} for event in events]
...
def list_tickets(event_id):
    ...
    tickets_list = [{"ticketId": ticket.ticket_id, "seatNumber": ticket.seat_number, "userId": ticket.user_id or "Unassigned", "price": ticket.price} for ticket in tickets]
...
def assign_ticket(ticket_id): #TODO add mutliple ticket functionality
    ...
    new_transaction = Transaction(ticket_id=ticket_id, user_id=user_id, price=ticket.price, payment_method_id=payment_method_id)
...
def remove_ticket(ticket_id):
    ...
    new_transaction = Transaction(ticket_id=ticket_id, user_id=user_id, price=price, payment_method_id=payment_method_id)
    db.session.add(new_transaction)
